...
public class MovieController {
    ...
    @Operation(
            ...
            description =
                    "Imports movies from a local `top250_min.json` file and enriches them with"
                        + " details from the OMDb API. Movies are saved into the database only if"
                        + " they don't already exist. This operation is **restricted to admin users"
                        + " only**.",
            ...
            for (ImdbMovieJsonDTO jsonMovie : movieList) {
                ...
                Movie movie =
                        Movie.builder()
                                ...
                                .year(
                                        omdbData.get("Year") != null
                                                ? Integer.parseInt((String) omdbData.get("Year"))
                                                : null)
                                .rating(
                                        omdbData.get("imdbRating") != null
                                                ? Double.parseDouble(
                                                        (String) omdbData.get("imdbRating"))
                                                : null)
